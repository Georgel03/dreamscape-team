...
import { DreamInputOptions } from '@/types/dreamMediaType';
interface DreamContextType {
  ...
  generateDream: (options?: DreamInputOptions) => Promise<void>;
  clearResults: () => void;
}
...
export function DreamProvider({ children }: { children: ReactNode }) {
  ...
  const generateDream = async (options?: DreamInputOptions) => {
    ...
    try {
      const result = await generateDreamAction(prompt, options);
