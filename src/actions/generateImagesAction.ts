...
async function uploadToStorage(base64Data: string, userId: string): Promise<string> {
  ...
  const { error } = await supabaseAdmin.storage.from('dream-media').upload(fileName, buffer, { contentType: 'image/png', upsert: true });
...
export async function generateImageVertex(prompt: string, userId: string): Promise<{ url: string, base64: string }> {
  try {
    ...
    const publicUrl = await uploadToStorage(base64, userId);
